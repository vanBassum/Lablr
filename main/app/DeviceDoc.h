...
inline constexpr const char* INSTRUCTIONS =
    ...
    "\n"
    "Printing\n"
    "  `print svg` renders a stored SVG and prints it on a DYMO LabelWriter "
    "attached to the USB host port. It is the same renderer `render svg` uses, "
    "so what the preview shows is what comes out.\n"
    "  Geometry is in PRINTER DOTS, not millimetres: at 300 DPI, one millimetre "
    "is 11.81 dots. There is no media layer yet, so YOU supply the numbers and "
    "nothing on the device knows what roll is loaded. Ask the user rather than "
    "guessing a label size.\n"
    "  `headWidth` is the PRINTER head width, not the label width, and defaults "
    "to 672 - the 300 DPI head. The head prints from its own left edge, so a "
    "narrower label needs `offsetX` to sit where the paper actually is.\n"
    ...
    "`warning` field means the label came out BLANK, which is almost always an "
    "SVG naming a font-family the device does not have - compare it against "
    "`render fonts`.\n"
    "  `print test` prints a built-in striped pattern with no filesystem and no "
    "renderer involved, which is how you tell a printer problem from a label "
    "problem.\n"
    "\n"
