...
| File | Purpose |
|------|---------|
| `Lablr-factory.bin` | Full image (bootloader + partitions + app + www) for initial flash |
| `Lablr-app.bin` | Firmware only, for OTA update via web UI |
| `Lablr-www.bin` | Web UI only, for updating the frontend independently |
