...
/// <summary>
/// Server-side label renderer (headless / AI print path). A deliberate second
/// renderer that mirrors the PWA canvas renderer (lablr-ui/src/services/render.ts):
/// text shrink-to-fit/wrap/align/valign with a margin safe-area, plus SVG
/// pictograms — thresholded to 1-bit and packed into a DYMO LabelWriter 450
/// raster job (same byte format as lablr-ui/src/dymo.ts).
///
/// NOTE: this can drift from the PWA preview (fonts/layout differ). That trade
/// was accepted to enable AI-initiated printing without a browser in the loop.
/// </summary>
public sealed class LabelRenderer
{
    ...
    /// <summary>Render a draft+template to a complete LW450 job (bytes for the bridge).</summary>
        ...
        LabelStock stock, Printer printer, string orientation)
    {
        ...
        using var label = new SKBitmap(Math.Max(1, w), Math.Max(1, h));
        ...
        try
        {
            int offX = (int)Math.Round((stock.OffsetCorrectionMm?.X ?? 0) * mmToDots);
            int offY = (int)Math.Round((stock.OffsetCorrectionMm?.Y ?? 0) * mmToDots);
