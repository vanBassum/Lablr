...
{
    public sealed record PrintDraftRequest(string DraftId, string? TemplateId, string? AgentId);
    ...
    public async Task<IActionResult> Draft([FromBody] PrintDraftRequest req, CancellationToken ct)
    {
        ...
        var r = await print.PrintDraftAsync(req.DraftId, req.TemplateId, req.AgentId, ct);
        return r.Ok
