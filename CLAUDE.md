...
- **Printer** — a YAML profile (`config/printers/`) of printer-level traits shared by every roll: the leading-edge **dead zone** (`topMarginMm` — the unprintable band where the head can't reach, because the gap sensor sits before it). Media declare which printers they fit via `media.printers`. (DPI + head width still live in the driver while there's a single printer; they move here with item 24.) The offset that corrects print placement is split: **Y dead-zone on the printer** (shared), **X position on the media** (per roll) — applied only at print time, never to the preview.
