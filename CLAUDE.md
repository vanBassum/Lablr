...
Lablr is ESP32 firmware (ESP-IDF v6.0, C++, FreeRTOS) with a React web UI, copied from [Strux](https://github.com/vanBassum/Strux) — the template — on 2026-09-18 and renamed, and re-synced with it since. Everything below still describes that foundation, because that is what the code here still is. `main/strux/` is the template's and stays generic: improvements travel both ways with upstream and its sibling forks, so a change made there should be one Strux would want, and a sync is a re-copy that re-applies only this repository's identity. Product code lives in `main/app/`, where nothing is owed to anyone else.
...
idf.py -DBOARD=<name> build           # select a board from main/hardware/boards/ (default: esp32s3_n16r8)
```

**One board: `esp32s3_n16r8`** — ESP32-S3 with 16 MB flash and 8 MB octal PSRAM, which is the product's hardware. The template's `esp32_devkit` and `esp32c3_supermini` were deleted rather than kept building: they are 4 MB parts with no PSRAM, and none of what this firmware is for fits on them. A board still needs *both* halves to build — `idf.py -DBOARD=<name> set-target <chip>`, then build — because `set-target` picks the chip and `-DBOARD` picks the pinout, and a board's `sdkconfig.defaults` cannot supply the chip (see the note below). Add `-B build_x -D SDKCONFIG=sdkconfig.x` to keep a second board's tree beside the default one instead of overwriting it.

**Flash size and the partition table are board facts, not project facts.** Each board's `sdkconfig.defaults` carries its own `CONFIG_ESPTOOLPY_FLASHSIZE_*MB` and `CONFIG_PARTITION_TABLE_CUSTOM_FILENAME`, and the root `sdkconfig.defaults` asserts neither — deliberately, so there is no default for a board to inherit by accident. Asserting them at the root is what made a 16 MB board impossible: the overlay wins on conflicts, so the root's 4 MB line stopped being in effect, and the drift guard below correctly refused the build for it. The guard was right; the assertion was in the wrong file. It still checks every line of every composed defaults file, and now names the board overlay when one does not take.
...
**The automated tests cover the two functions that need no device**, and nothing else: `path::ResolveUnder` ([main/lib/common/PathResolve.h](main/lib/common/PathResolve.h)), the only barrier between a path a client chose and the rest of the VFS, and the raster pair in [main/app/PrintManager/Raster.h](main/app/PrintManager/Raster.h), where a wrong byte shears a printed label. They live in [test/host/test_pure.cpp](test/host/test_pure.cpp), have no framework, and are one `g++` away:

```bash
g++ -std=c++17 -Wall -Wextra -Werror -I main/lib/common -I main/app/PrintManager     test/host/test_pure.cpp -o test_pure && ./test_pure
```

That list is short on purpose. Everything else here needs a printer, a network or a flash partition, and **resisting the urge to mock one is the point** — a mocked printer proves the mock. [.github/workflows/ci.yml](.github/workflows/ci.yml) runs those tests, the frontend typecheck and build, and the firmware, on every push and pull request; `release.yml` still owns `v*` tags. Real verification is building, flashing, and driving the device over its own wire:
...
- **A command runs to completion on its transport's task, and the browser socket has exactly one.** `esp_http_server` services every socket from a single task, and `RunCommandSession` dispatches synchronously on it, so while a handler is working no other WebSocket client gets a reply. `print svg` is the one that makes this visible: render plus up to 30 s of USB send (the timeout; a real 25 mm label is 1-3 s). This is **accepted, not overlooked** — the device is one printer with one user, log broadcasts go out on `ConsoleManager`'s own task and keep flowing during a job, and the two fixes both cost more than the problem: an async `print svg` returning a job id breaks "one call, one label", which is what `DeviceDoc` promises a caller that has never seen this repo, and deferring a reply to a worker task means a session that outlives its handler. Do not add a `vTaskDelay` or any other sleep inside a handler for the same reason — it stalls every client, which is why the login rate limit refuses immediately instead of answering slowly. What this DOES forbid is a page that fans out many commands at once: see `docs/reasoning/2026-09-18-18h25-the-strip-does-not-need-an-honest-picture-only-the-preview-does.md`, where eighty queued thumbnail renders made the whole device unresponsive on page load.
...
**Three things decide where a label lands, and they have different owners.** This was one signed pair of offsets on a medium until 2026-09-20, which meant nudging artwork silently reported a shorter printable label - see `docs/reasoning/2026-09-20-18h40-one-signed-number-was-answering-two-questions.md`.

| Owner | What it is | Where |
|---|---|---|
| **Printer** | dpi, head width, and the strip the mechanism cannot reach (`deadLeftUm`, `deadTopUm`). Same for every roll. | `/printers/<id>.json`, `PrinterManager`. One entry is **compiled in** (`dymo450`), so a device with no files still prints; a file overrides it, and `print.printer` selects. |
| **Medium** | size, plus `marginLeftUm`/`marginTopUm` - what THIS roll loses beyond the machine. Non-negative. | `/media/<id>.json` |
| **Alignment** | `alignXUm`/`alignYUm` - where the artwork sits. Signed. The only one that is a preference. | the medium |

    printable = size - printerDead - mediumMargin
    placement = align - printerDead - mediumMargin

Alignment is in the second and **not** the first: moving artwork never changes how much label there is. The arithmetic is [DotGeometry.h](main/lib/common/DotGeometry.h) rather than either manager, so `media get`, the preview and a real print cannot disagree - and so the host tests reach it.

**Backward compatibility is load-bearing here.** A medium still carrying `offsetXUm`/`offsetYUm` has them honoured as the *finished placement*, verbatim, and gets its printable area from the printer; the built-in dead zone is seeded with square25's own measured 1016/3133 um, so that roll's numbers are unchanged to the dot. `media set` migrates it, and that migration **converts**: `align = oldOffset + dead + margin`, because an old offset named the finished position while alignment is measured from the first reachable dot. Copying instead of converting shifts every calibrated roll, which looks like a printer fault. A host test pins it: square25 migrates to alignment **zero**.

**The calibration grid is a design, not a raster.** `print calibrate` still exists and still writes head-coordinate bars straight into the job - that is what tells a printer fault from a label fault, because it uses no filesystem, renderer or medium. But the grid you calibrate WITH is `-pattern calibration` on `render svg` and `print svg`: generated by [SvgPatterns.h](main/lib/common/SvgPatterns.h) at the medium's dot size, centre-origin, rings every 25 dots and heavier every 100, and put through the same renderer, fit and placement a label gets. So it can be previewed for free, and the Print button prints exactly the previewed picture. `-pattern test` is the same deal for the test pattern.
...
- **Font support needs espressif/thorvg 1.1.2 or newer.** Every earlier release passes `-D__linux__`, and ThorVG's font loader takes that to mean mmap exists. This was a pinned fork until 2026-09-20; the one-token fix is upstream and released now, so [main/idf_component.yml](main/idf_component.yml) is the plain registry component with its floor at the version that carries it. The root CMakeLists still refuses a build whose resolved component defines the token, with instructions, rather than failing inside the dependency.
