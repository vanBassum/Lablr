...
public:
    void Init()
    {
        // if constexpr block (not an early return) so the GPIO code — incl. the
        // shift by LED_PIN — is fully discarded when the pin is disabled (-1).
        if constexpr (BoardConfig::LED_PIN >= 0)
        {
            gpio_config_t cfg = {};
